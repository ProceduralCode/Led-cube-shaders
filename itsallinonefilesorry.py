...
class Shader:
	...
	"""
	def __init__(self, shape = 'point', param = None, lifespan = 1, spawn_inter = 0.1, gen_color = None):
		random.seed()
		self.shape = shape
		self.param = param
	...
	def add_particles(self):
		...
		if all([(self.curr_time - spawn_time) > self.spawn_inter for _, _, _, _, spawn_time in self.points]):
...
def emersons_favorites(name = 'green_stars'):
	...
	if name == 'green_stars':
		def gen_color(seed, age):
			...
			return co
		return Shader(shape = 'particle point', lifespan = 8, spawn_inter = 0.03, gen_color = gen_color)
	elif name == 'rainbow_sparkles':
		def gen_color(seed, age):
			random.seed(seed)
			t = time.time()
			period = 4
			rainbow = [(-2 * abs(((t + i * period / 3 + random.random() * 0.15 * period) % period / period) - 0.5) + 1) for i in range(3)]
			co = [rainbow[i] * (-2 * abs(age - 0.5) + 1 ) for i in range(3)]
			return co
		return Shader(shape = 'particle point', lifespan = 1, spawn_inter = 0.005, gen_color = gen_color)
		...
		def gen_color(seed, age):
			...
			co = [c1[i] * (-2 * abs(age - 0.5) + 1) * variance for i in range(3)]
			return co
		return Shader(shape = 'particle rain', param = 4, lifespan = 3, spawn_inter = 0.08, gen_color = gen_color)
